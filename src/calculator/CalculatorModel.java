...
/**
 * This is the model of this MVC implementation of a calculator. It performs the
 * functions of the calculator and keeps track of what the user has entered.
 * 
 * @author Steven R. Corbin
 */
public class CalculatorModel {
	/**
	 * This is the numeric value of the number the user is entering, or the
	 * number that was just calculated.
	 */
	...
	/**
	 * Updates the values maintained by the calculator based on the button that
	...
	public void update(String text) {
		...
		if (text.length() == 1 && "0123456789".indexOf(text) >= 0) {
			...
			if (text.equals(".")) {
				...
				displayValue = Double.valueOf(Math.E);
				displayString = "" + displayValue;
			} else if (text.contains("pi")) {
				displayValue = Double.valueOf(Math.PI);
				displayString = "" + displayValue;
				...
				if (operation.equals("+")) {
					...
					displayValue = Math.tan(internalValue);
				} else if (operation.equals("RAD")) {
					displayValue = Math.toRadians(internalValue);
				} 
				displayString = "" + displayValue;
